...
use crate::error::{ShareRelayError, SdkError};
...
fn validate_share_id(value: &str, field: &str) -> Result<(), SdkError> {
    ...
    let ok = value.bytes().all(|b| b.is_ascii_alphanumeric() || b == b'-' || b == b'_');
    if !ok {
        return Err(SdkError::ShareRelay(ShareRelayError::Unexpected(400)));
    }
...
impl<C: ProviderHttpClient> ShareRelayClient<C> {
    ...
    pub async fn create_b1(
        ...
        let resp = self.client.request(req).await.map_err(|e| SdkError::Api(e.to_string()))?;
        match resp.status {
            200 | 201 => {
    ...
    pub async fn get_b1(&self, share_id: &str) -> Result<B1GetResponse, SdkError> {
        validate_share_id(share_id, "share_id")?;
        let req = ProviderHttpRequest::get(format!("{}/relay/share/b1/{}", self.base_url, share_id));
        let resp = self.client.request(req).await.map_err(|e| SdkError::Api(e.to_string()))?;
        match resp.status {
            200 => {
                let v: serde_json::Value = serde_json::from_slice(&resp.body)
    ...
    pub async fn revoke_b1(&self, share_id: &str, owner_token: &str) -> Result<(), SdkError> {
        ...
        let req = ProviderHttpRequest::delete(format!("{}/relay/share/b1/{}", self.base_url, share_id))
    ...
    pub async fn get_b2(&self, share_id: &str) -> Result<Vec<u8>, SdkError> {
        validate_share_id(share_id, "share_id")?;
        let req = ProviderHttpRequest::get(format!("{}/relay/share/b2/{}", self.base_url, share_id));
        let resp = self.client.request(req).await.map_err(|e| SdkError::Api(e.to_string()))?;
        match resp.status {
            200 => {
                // Count bytes only on successful download.
                self.bytes_recv.fetch_add(resp.body.len() as u64, Ordering::Relaxed);
                Ok(resp.body)
            }
            404 => Err(SdkError::ShareRelay(ShareRelayError::NotFound)),
            410 => Err(SdkError::ShareRelay(ShareRelayError::Revoked)),
    ...
    pub async fn revoke_b2(&self, share_id: &str, owner_token: &str) -> Result<(), SdkError> {
        validate_share_id(share_id, "share_id")?;
        validate_header_value(owner_token, "owner_token")?;
        let req = ProviderHttpRequest::delete(format!("{}/relay/share/b2/{}", self.base_url, share_id))
            .header(("X-Owner-Token".into(), owner_token.into()));
        let resp = self.client.request(req).await.map_err(|e| SdkError::Api(e.to_string()))?;
        match resp.status {
            204 => Ok(()),
...
fn json_i64(v: &serde_json::Value, key: &str) -> Result<i64, SdkError> {
    v.get(key)
        .and_then(|f| f.as_i64())
        .ok_or_else(|| SdkError::Api(format!("missing or invalid '{key}' field in relay response")))
}
...
mod tests {
    ...
    async fn create_b1_ok() {
        let c = MockClient { status: 201, body: ok_body("test-id") };
        let relay = ShareRelayClient::new(c, "https://relay.example.com");
        let resp = relay
            .create_b1("test-id", "https://gdrive.com/file", 86400, "relay_auth=jwt")
            .await
            .unwrap();
    ...
    async fn create_b1_conflict() {
        let c = MockClient { status: 409, body: vec![] };
        let relay = ShareRelayClient::new(c, "https://relay.example.com");
        let err = relay
            .create_b1("x", "https://gdrive.com/file", 86400, "relay_auth=jwt")
            .await
            .unwrap_err();
        assert!(matches!(err, SdkError::ShareRelay(ShareRelayError::Conflict)));
    }

    #[tokio::test]
    async fn create_b1_unauthorized() {
        let c = MockClient { status: 401, body: vec![] };
        let relay = ShareRelayClient::new(c, "https://relay.example.com");
        let err = relay
            .create_b1("x", "url", 3600, "bad_cookie")
            .await
            .unwrap_err();
        assert!(matches!(err, SdkError::ShareRelay(ShareRelayError::Unauthorized)));
    }

    #[tokio::test]
    async fn get_b1_not_found() {
        let c = MockClient { status: 404, body: vec![] };
        let relay = ShareRelayClient::new(c, "https://relay.example.com");
        let err = relay.get_b1("missing").await.unwrap_err();
        assert!(matches!(err, SdkError::ShareRelay(ShareRelayError::NotFound)));
    }

    #[tokio::test]
    async fn get_b1_revoked() {
        let c = MockClient { status: 410, body: vec![] };
        let relay = ShareRelayClient::new(c, "https://relay.example.com");
        let err = relay.get_b1("revoked-id").await.unwrap_err();
        assert!(matches!(err, SdkError::ShareRelay(ShareRelayError::Revoked)));
    }
    ...
    async fn revoke_b1_ok() {
        let c = MockClient { status: 204, body: vec![] };
        let relay = ShareRelayClient::new(c, "https://relay.example.com");
    ...
    async fn upload_b2_too_large() {
        let c = MockClient { status: 413, body: vec![] };
        let relay = ShareRelayClient::new(c, "https://relay.example.com");
        let err = relay
            .upload_b2("x", vec![0x07u8; 200], 3600, "relay_auth=jwt")
            .await
            .unwrap_err();
        assert!(matches!(err, SdkError::ShareRelay(ShareRelayError::TooLarge)));
    }

    #[tokio::test]
    async fn upload_b2_rate_limited() {
        let c = MockClient { status: 429, body: vec![] };
        let relay = ShareRelayClient::new(c, "https://relay.example.com");
        let err = relay
            .upload_b2("x", vec![0x07u8], 3600, "relay_auth=jwt")
            .await
            .unwrap_err();
        assert!(matches!(err, SdkError::ShareRelay(ShareRelayError::RateLimited)));
    }

    #[tokio::test]
    async fn get_b2_ok() {
        let fake_v7 = vec![0x07u8; 2048];
        let c = MockClient { status: 200, body: fake_v7.clone() };
        let relay = ShareRelayClient::new(c, "https://relay.example.com");
    ...
    async fn share_id_with_crlf_is_rejected() {
        ...
        assert!(matches!(err, SdkError::ShareRelay(ShareRelayError::Unexpected(400))));
    }
    ...
    async fn owner_token_with_crlf_is_rejected() {
        let c = MockClient { status: 204, body: vec![] };
        let relay = ShareRelayClient::new(c, "https://relay.example.com");
        let err = relay.revoke_b1("valid-id", "tok\r\nX-Injected: evil").await.unwrap_err();
        assert!(matches!(err, SdkError::ShareRelay(ShareRelayError::Unexpected(400))));
    }
    ...
    async fn relay_cookie_with_crlf_is_rejected() {
        ...
        let relay = ShareRelayClient::new(c, "https://relay.example.com");
        let err = relay
            .create_b1("valid-id", "https://example.com/file", 3600, "relay_auth=x\r\nevil: hdr")
            .await
            .unwrap_err();
        assert!(matches!(err, SdkError::ShareRelay(ShareRelayError::Unexpected(400))));
    }
