...
use crate::relay_auth::{verify_relay_cookie, AppState, RELAY_COOKIE_NAME_STATS};
...
pub async fn ingest_stats(
    ...
    if let Err(_e) = state
        .stats_store
        .apply_batch(&device_hash, &payload.events, now)
    {
...
mod tests {
    ...
    fn hash() -> [u8; 32] {
        StatsStore::hash_device(
            b"test_hmac_key_32_bytes_minimum!!",
